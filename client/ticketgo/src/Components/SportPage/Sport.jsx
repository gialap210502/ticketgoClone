    ...
    return (
        <div className='container' style={{ paddingTop: '10px', paddingBottom: '35px' }}>
            <div className='row'>
                <div className='col-6'>
                    <ul className=" rounded breadcrumb" style={{ width: '160px' }}>
                        ...
                        <li><span>Thể thao</span></li>
                    </ul>
                </div>
            </div>
            <div className='row'>
                {listItems?.map((item) => {
                    if (!itemDisplayed && item.fields.Category && item.fields.Category.includes("Sport")) {
                        itemDisplayed = true;
                        return (
            ...
            <div className="row" style={{ float: 'right', color: '#ff672a', fontSize: '17px', fontFamily: 'open sans' }}>
                <p>Tổng số : {listItems.filter(item => item.fields.Category && item.fields.Category.includes("Sport")).length} Sự kiện</p>
            </div>
            <div style={{ height: '45px' }}></div>
            <div className="row" style={{ display: 'flex' }} >
                {listItems?.map((item) => {
                    if (item.fields.Category && item.fields.Category.includes("Sport")) {
                        return (
...
export default EventSportDisplay;  